...
if __name__ == '__main__':
    ...
    lst.close()
    obj.write(addArray[0].upper()[2:].zfill(6))
    obj.write("\n")
    obj.write(str(0).zfill(6))
    obj.write("\n")
    for ctr, i in enumerate(lines):
        lbl, instr, opr, cmm = i
        if instr in ["RESW", "RESB"]:
            obj.write("!")
            obj.write("\n")
            obj.write(addArray[ctr + 1].upper()[2:].zfill(6))
            obj.write("\n")
            obj.write(str(0).zfill(6))
            obj.write("\n")
        elif instr is "END":
